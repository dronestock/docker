...
func (b *stepBoost) from(line string) (new string, err error) {
	if uri, params, pe := b.parse(line); nil != pe {
		...
		size := len(strings.Split(uri.Host, slash))
		host := gox.Ifx(1 == size, fmt.Sprintf("%s/%s", library, uri.Host), uri.Host)
		new = fmt.Sprintf("%s %s/%s%s%s", from, b.Boost.Mirror, host, uri.Path, gox.If("" != params, params))
	}
